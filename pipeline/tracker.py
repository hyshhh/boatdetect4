...
class TrackInfo:
    ...
    clarity: str = ""              # 弦号清晰度："clear" | "blurry" | ""
...
class TrackManager:
    ...
    def bind_hull_box(
        self,
        track_id: int,
        hull_box: list[float] | None,
    ) -> None:
        """绑定弦号框相对坐标 [rx1, ry1, rx2, ry2]。"""
        with self._lock:
            if track_id in self._tracks:
                self._tracks[track_id].hull_box = hull_box
    ...
    def bind_clarity(
        ...
        """绑定弦号清晰度（"clear" | "blurry" | ""）。"""
        with self._lock:
            if track_id in self._tracks:
                self._tracks[track_id].clarity = clarity
